...
function updateCourt(courtLetter) {
    ...
    const mType = document.getElementById(`type-${courtLetter}`).value; // 新增這行：抓取賽制
    ...
    .then(() => alert(`Court ${courtLetter} (${mType}) updated!`))
...
async function refreshHomeData() {
    ...
    try {
        ...
        data.forEach(match => {
            ...
            const typeEl = document.getElementById(`home-type-${letter}`);

            if (typeEl) {
                typeEl.innerText = match.matchType ? `(${match.matchType})` : "";
                typeEl.fontWeight = "bold";
                typeEl.fontSize = "x-large";
                typeEl.style.marginLeft = "10px";
